...
"$UVICORN_BIN" logger.main:app --host 0.0.0.0 --port 8003 > logger.log 2>&1 &
sleep 2

# 2. Start Character Service (Port 8001)
echo " [2/4] Starting Character Service on port 8001..."
"$UVICORN_BIN" main:app --app-dir classifier/character-classifier --host 0.0.0.0 --port 8001 > character_service.log 2>&1 &
sleep 5 # Wait for model loading

# 3. Start Word Service (Port 8002)
echo "[3/4] Starting Word Service on port 8002..."
"$UVICORN_BIN" main:app --app-dir classifier/word-classifier --host 0.0.0.0 --port 8002 > word_service.log 2>&1 &
sleep 5 # Wait for EasyOCR loading

# 4. Start Gateway (Port 8000)
echo "[4/4] Starting Gateway on port 8000..."
"$UVICORN_BIN" gateway.main:app --host 0.0.0.0 --port 8000 > gateway.log 2>&1 &
