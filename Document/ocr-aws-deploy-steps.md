...
After the initial deployment, we added persistence for images and predictions.

## 1. IAM Permissions Update
The `web-app-docker-admin` user and the ECS Task Role need access to S3 and RDS.
- **Policies added to `web-app-docker-admin`**:
  - `AmazonS3FullAccess`
  - `AmazonRDSFullAccess`
  - `AmazonEC2ContainerRegistryFullAccess`

- **ECS Task Role setup**:
  1. Created a new IAM Role: `ocr-task-role`.
  2. Trusted Entity: `ecs-tasks.amazonaws.com`.
  3. Policies attached: `AmazonS3FullAccess`, `AmazonRDSFullAccess`.
  4. Assigned this role to the **Task Role** field in the ECS Task Definition.

## 2. Infrastructure Creation (AWS CLI)
...
## 4. Code & Image Update
Updated the Gateway service to use `boto3` (S3) and `SQLAlchemy` (RDS).

### Build and Push Updated Gateway
```bash
...
Created a new revision of `ocr-gateway-task` with these critical settings:
...
To verify that the cloud storage is working correctly, use these commands from your local terminal.
...
If connection fails:
1. Ensure the RDS instance is set to **Public Access: Yes**.
2. Ensure your Home IP is added to the **RDS Security Group** Inbound Rules for Port 5432.

# Phase 6: Monitoring & Observability (CloudWatch)

To ensure the system is healthy and to track performance, we use **Amazon CloudWatch**.

## 1. CloudWatch Logs (Centralized Logging)
All microservices are configured to send their logs to CloudWatch. Based on our environment, we have the following Log Groups:
- **Gateway**: `/ecs/ocr-gateway` and `/ecs/ocr-gateway-task`
- **Character Service**: `/ecs/ocr-character-service` and `/ecs/ocr-character-task`
- **Word Service**: `/ecs/ocr-word-service` and `/ecs/ocr-word-task`
- **Cluster Performance**: `/aws/ecs/containerinsights/ocr-cluster/performance`

These logs are essential for debugging:
- Database connection timeouts (RDS)
- S3 "Unable to locate credentials" errors
- FastAPI application crashes

## 2. CloudWatch Alarms & Metrics
We monitor the following for system stability:
- **ServiceDiscovery_HealthCheckStatus**: Monitors if backend services are reachable.
- **CPU & Memory Utilization**: Monitored via Container Insights to prevent out-of-memory errors during heavy inference.
- **RDS Connections**: Tracking database load.
![cloud_watch](image-20.png)
---
# Results & Validation
- **Images**: Uploaded images are stored in `s3://ocr-storage-anas-470895881101/uploads/`.
- **Predictions**: OCR results are saved in the `predictions` table in RDS.
- **Visibility**: The Gateway remains public via the ALB, but all backend traffic and database access are restricted to the internal VPC network.
